    ...
    PausableUpgradeable
{
    ...
    IBEP20 public constant stakingToken = IBEP20(0x4C16f69302CcB511c5Fac682c7626B9eF0Dc126a); //Date Token
    ...
    IBEP20 public rewardsToken; // Date/bnb flip
    ...
    function initialize() external initializer {
        ...
        IBEP20(ETH).safeApprove(address(QUICK_ROUTER), ~uint(0));
        stakingToken.safeApprove(address(QUICK_ROUTER), ~uint(0));
    }
    ...
    modifier onlyValidator() {
        require(msg.sender == tx.origin
            || _whitelist[msg.sender], "DatePool: only whitelist");
        _;
    }
        ...
        )
    {
        ...
        (uint DatePrice, ) = priceCalculator.valueOfAsset(address(stakingToken), 1e18);
        (uint flipPrice, ) = priceCalculator.valueOfAsset(address(rewardsToken), 1e18);

        _usd = 0;
        _Date = 0;
        _bnb = rewardPerTokenPerSecond.mul(365 days).mul(flipPrice).div(DatePrice);
    }
    ...
    function _deposit(uint amount, address _to) private nonReentrant whenNotPaused updateReward(_to) {
        require(amount > 0, "amount");
    ...
    function _flipToWETH(uint amount) private returns (uint reward) {
        (uint rewardDate, ) = QUICK_ROUTER.removeLiquidity(
        ...
        path[1] = ETH;
        QUICK_ROUTER.swapExactTokensForTokens(rewardDate, 0, path, address(this), block.timestamp);
    ...
    function setRewardsToken(address _rewardsToken) external onlyOwner {
        ...
        IBEP20(_rewardsToken).safeApprove(address(QUICK_ROUTER), ~uint(0));
    }
